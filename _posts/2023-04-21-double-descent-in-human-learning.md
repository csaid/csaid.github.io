...
I’m not ready to say that U-shape learning in human brains is _identical_ to double descent in machine learning. Unlike double descent, the performance degradations in U-shaped human learning are confined to local subsets of tasks rather than global performance. (See [Saxe et al. (2019)](https://doi.org/10.1073/pnas.1820226116) for how this can emerge in neural nets.) But it is still a great example of how switching between different regimes can cause performance degradations, whether in artificial neural networks or in the largest neural net in the known universe.
